...
== Ausblick
Obwohl schon einige Optimierungen vorgenommen wurden, besteht noch viel Potential zur Verbesserung der Performance. Die Implementierung könnte auf eine hardwarenähere Sprache portiert werden, um die Effizienz zu steigern. Auch die Implementierung der Effekte in einer Shader-Sprache könnte die Performance verbessern. Interessant wäre auch die Effekte eher zu simulieren als zu emulieren, um die künstlerische Freiheit zu erhöhen. Beispielsweise könnten die Grain-Körner einzeln simuliert werden, um so verschiedene Körnungen zu erzeugen. Dies wäre jedoch mit einem erhöhten Rechenaufwand verbunden und mutmaßlich nicht in Echtzeit umsetzbar.

== Anwendungsbereiche
Aufgrund der Echtzeitfähigkeit der Effekte eignet sich die Implementierung für die Anwendung in der Live-Produktion von Bewegtbildern. Auch in Videospielen könnten die Effekte eingesetzt werden, jedoch wäre dafür eine Implementierung in einer Hardwaren näheren Sprache sinnvoll um die Effizienz für diesen Anwendungsfall zu steigern. Zusätzlich könnte die Implementierung aufgrund ihres Fokusses auf die Performance in abgewandelter Form sinnvoll für die Postproduktion auf schwächerer Hardware eingesetzt werden, beispielsweise in der Bearbeitung von Videos auf dem Smartphone.
