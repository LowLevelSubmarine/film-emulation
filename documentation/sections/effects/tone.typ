...
=== Implementierung
Um unterschiedliche Farbtöne eines Bildes getrennt voneinander anzupassen, ist es zunächst sinnvoll, das Bild in einen Farbraum zu konvertieren, in dem die Farbtöne auf einen einzigen Wert abgebildet werden (z.B. HSV, Farbton ist "Hue"). So können die Farbtöne nach einem Mapping durch ein LUT als Maske für weitere Verarbeitungen verwendet werden (@fig:tone-effect-hue-filter). In diesem Fall wird eine Maske für orange Farbtöne erstellt.
...
Um die Farbtöne schlussendlich ohne auffällige Artefakte verändern zu können, muss zunächst sichergestellt werden, dass intensivere Farben mit höherer Sättigung ("Saturation") und Luminanz ("Value") auch entsprechend intensiver verfärbt werden. Dafür wird die Maske für orange Farbtöne mit dem Sättigungs- und Luminanz-Kanal multipliziert. In einem kleinen Zwischenschritt wird nun die entstandene Maske in Kontrast und Helligkeit für die Feinabstimmung der Intensität angepasst. Anschließend wird die Maske in einen 3-Kanal-Buffer kopiert, da die weiteren Schritte mit Farben arbeiten. Außerdem wird eine invertierte Kopie der Maske erstellt, um auch die kälteren Farbtöne getrennt anpassen zu können. Im letzten Schritt werden die Farben des Bildes anhand der Masken angepasst. Dazu wird das Eingabebild zunächst mit den jeweiligen Masken multipliziert, um nur die Werte zu erhalten, die angepasst werden sollen. Die Anpassung selbst geschieht durch eine Multiplikation der nun entstandenen Buffer mit den jeweiligen Farbtönen. Zurückgegeben wird schließlich die Summe der beiden Buffer und des Eingabebildes.
